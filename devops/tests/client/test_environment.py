...
class TestDevopsEnvironment(driverless.DriverlessTestCase):
    ...
    def setUp(self):
        ...
        self.nc_mock_inst.get_subordinate_ip_by_mac.side_effect = self.mac_to_ip.get
        ...
        self.cr_sl_conf_mock = self.patch(
            'devops.helpers.templates.create_subordinate_config')
        self.cr_sl_conf_mock.return_value = self.subordinate_conf
    ...
    def test_add_subordinates(self):
        nodes = self.denv.add_subordinates(
            nodes_count=1)

        self.cr_sl_conf_mock.assert_called_once_with(
            subordinate_name='subordinate-01',
        ...
        assert len(nodes) == 1
        assert nodes[0].name == 'subordinate-01'
    ...
    def test_get_node_ip(self):
        ...
        subordinate = self.group.add_node(
            name='subordinate-01',
            role='fule_subordinate',
            interfaces=[dict(
                label='eth0',
                l2_network_device='admin',
                interface_model='e1000',
        ...
        eth0 = subordinate.get_interface_by_network_name('admin')
        eth0.mac_address = '64:52:dc:96:12:cc'
        eth0.save()

        ip = self.denv.get_node_ip('subordinate-01')
    ...
    def test_get_node_remote(self):
        ...
        key = self.paramiko_mock.RSAKey.from_private_key.return_value
        keys = [key, key]
        remote = self.denv.get_node_remote('subordinate-01')
        assert remote is ssh
    ...
    def test_get_curr_time(self):
        ...
        t = self.denv.get_curr_time(node_names=['admin', 'subordinate-01'])
        assert t is self.ntpgroup_inst.get_curr_time.return_value
